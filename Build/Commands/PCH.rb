...
module Build
    ...
    module PCH
        ...
        def self.generate_pch(fname, global, configs)
            ...
            pch.puts!('/*')
            pch.puts!('** Generated by %s v%s', Build::Project, Build::VERSION)
            pch.puts!('*/')

            pch.puts!('#ifdef PLATFORM_%s', os_name.to_s.upcase)

            pch.puts!('// Global system includes')
            PCH.make_include(pch, global.systemHeaders, system: true)
            pch.puts!('// Global project includes')
