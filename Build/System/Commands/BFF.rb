...
module Build
    ...
    make_command(:bff, 'BFF generator') do |&namespace|
        source = BFF::Source.new(Build.bff_path, minify: Build.Minify)

        source.comment!("BFF generated by %s v%s", File.basename($0), VERSION)
        source.func!('Settings') do
            set!('AllowDBMigration_Experimental', true)
            set!('CachePath', File.join($OutputPath, 'BuildCache'))
        ...
        BFF.make_targets(source, Build.get_environments(), namespace[].all)
        BFF.export(source)
    end

    make_prerequisite(:bff_digest) do
        if File.exist?(Build.bff_path)
            MemFile.file_digest(Build.bff_path)
        else
            nil
        end
    end
    make_prerequisite(:modified_fileslist_digest) do
        if File.exist?(BFF.modified_fileslist_path)
            MemFile.file_digest(BFF.modified_fileslist_path)
        else
            nil
        end
    end
    ...
    def self.bff_path()
        ext = File.extname($0)
        File.join($IntermediatePath, File.basename($0, ext) << '.bff')
    end

    module BFF

        def self.export(source)
            digest = source.digest
            if digest != Build.bff_digest
                Log.info 'export BFF file "%s"', source.filename
                source.write_to_disk()
                Build.restore_prerequisite('Build.bff_digest', digest)
            else
                Log.verbose 'skip saving bff since "%s" did not change', source.filename
            end
        end

        def self.make_targets(bff, environments, targets)
            platforms = Set.new
            configs = Set.new
        ...
        def self.target_base(bff, env, target, target_alias, expanded)
            ...
            bff.once?(target_source) do
                ...
                bff.struct!(target_source) do
                    ...
                    set!('CompilerInputFiles', env.source_path(target.isolated_files))
                    if target_unity
