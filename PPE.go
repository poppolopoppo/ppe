...
func (x BuildModulesGeneratedHeader) Generate(bc BuildContext, generated *BuildGenerated, dst io.Writer) error {
	unit, err := GetBuildUnit(x.Target)
	if err != nil {
		return err
	}
	...
	for _, it := range unit.LinkDependencies {
	...
	if len(unit.LinkDependencies) > 0 {
	...
	if len(unit.RuntimeDependencies) > 0 {
		fmt.Fprintln(dst, "    auto& registry = FModuleDynamicRegistration::Get();")
		for _, it := range unit.RuntimeDependencies {
	...
	fmt.Fprintf(dst, "constexpr std::array<FStringView, %d> DependencyList{\n",
		len(unit.LinkDependencies)+len(unit.RuntimeDependencies))
	for _, it := range unit.LinkDependencies {
