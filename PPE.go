...
func (x BuildModulesGeneratedHeader) Generate(bc BuildContext, generated *BuildGenerated, dst io.Writer) error {
	...
	filterModule := func(ta TargetAlias) bool {
		...
		LogPanicIfFailed(LogPPE, err)
		switch moduleType := module.GetModule().ModuleType; moduleType {
		case MODULE_LIBRARY:
			return true
		case MODULE_EXTERNAL, MODULE_HEADERS, MODULE_PROGRAM:
		default:
			UnexpectedValuePanic(moduleType, moduleType)
		}
		return false
	}

	linkDependencies := RemoveUnless(filterModule, unit.LinkDependencies...)
	runtimeDependencies := RemoveUnless(filterModule, unit.RuntimeDependencies...)
