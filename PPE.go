...
func (x BuildModulesGeneratedHeader) Generate(bc BuildContext, generated *BuildGenerated, dst io.Writer) error {
	...
	filterModule := func(ta TargetAlias) bool {
		...
		switch moduleType := module.GetModule().ModuleType; moduleType {
		case MODULE_LIBRARY:
			return true
		case MODULE_EXTERNAL, MODULE_HEADERS, MODULE_PROGRAM:
		}
		return false
	}

	linkDependencies := unit.LinkDependencies.RemoveUnless(filterModule)
	runtimeDependencies := unit.RuntimeDependencies.RemoveUnless(filterModule)

	fmt.Fprintln(dst, "#pragma once")
	fmt.Fprintln(dst, "// Build modules generated by "+CommandEnv.Prefix()+" - "+x.Version)
	fmt.Fprintln(dst, "#include \"Core_fwd.h\"")
	...
	for _, it := range linkDependencies {
	...
	if len(runtimeDependencies) > 0 {
		...
		for _, it := range runtimeDependencies {
...
func makePPE_Common(rules *ModuleRules) error {
...
func makePPE_Internal(rules *ModuleRules) error {
	...
	case PCH_DISABLED:
		// nothing TODO
	default:
		return fmt.Errorf("unsupported PCH module type: %v", rules.PCH)
	}
...
func makePPE_External(rules *ModuleRules) error {
	if err := makePPE_Common(rules); err != nil {
		return err
	}
...
func makePPE_Module(rules *ModuleRules) error {
	if err := makePPE_Common(rules); err != nil {
		return err
	}
	if err := makePPE_Internal(rules); err != nil {
		return err
	}
...
func makePPE_Program(rules *ModuleRules, usage UsageType) error {
	if err := makePPE_Common(rules); err != nil {
		return err
	}
	if err := makePPE_Internal(rules); err != nil {
...
func main() {
	...
	for _, usage := range UsageTypes() {
		RegisterArchetype("PPE/Program/"+(string)(usage), func(rules *ModuleRules) error {
			return makePPE_Program(rules, usage)
		})
	}
