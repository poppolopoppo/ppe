...
func (x BuildVersionHeader) Generate(bc BuildContext, env *CompileEnv, u *Unit, dst io.Writer) error {
	sourceControl := SourceControlBuilder.Need(bc)
...
func (x BuildModulesHeader) Generate(bc BuildContext, env *CompileEnv, u *Unit, dst io.Writer) error {
	...
	if u.RuntimeDependencies.Len() > 0 {
		...
		for _, x := range u.RuntimeDependencies.Slice() {
			fmt.Fprintf(dst, "    registry.RegisterLibrary(\"%s\", \"%s\", L\"%s\");\n",
				...
				env.GetBinariesOutput(x.ModuleAlias().String(), PAYLOAD_SHAREDLIB))
		}
	}
	fmt.Fprintln(dst, "}")
