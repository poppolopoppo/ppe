...
func (g GlslangGeneratedHeader) Generate(ctx GeneratorContext, dst io.Writer) error {
	...
	zip := getGlslangExtractor(downloader, extractDir)
	ctx.DependsOn(zip)
	...
	includeDir := zip.Destination.Folder("Include")
...
func getGlslangExtractor(download *Downloader, extractDir Directory) *ZipExtractor {
