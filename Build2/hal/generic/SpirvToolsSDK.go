...
func (g SpirvToolsGeneratedHeader) Generate(ctx GeneratorContext, dst io.Writer) error {
	...
	zip := getSpirvToolsExtractor(downloader, extractDir)
	ctx.DependsOn(zip)
	...
	includeDir := zip.Destination.Folder("install", "include")
	includeRe := MakeGlobRegexp(spirvToolsGlobIncludes...)
	for _, x := range zip.ExtractedFiles {
		rel := x.Relative(zip.Destination)
...
func getSpirvToolsExtractor(download *Downloader, extractDir Directory) *ZipExtractor {
	return NewZipExtractor(
