...
void MergeDuplicateVertices(GenericVertex& vertices, const MemoryView<u32>& indices) {
    ...
    STACKLOCAL_POD_ARRAY(u32, temporary, vertexCount*3);
    const MemoryView<u32> vertexHashes = temporary.SubRange(0, vertexCount);
    const MemoryView<u32> vertexSorted = temporary.SubRange(vertexCount, vertexCount);
    const MemoryView<u32> vertexReindexation = temporary.SubRange(vertexCount*2, vertexCount);
    {
        const u8 *vertexPtr = vertices.Destination().Pointer();
        for (u32 i = 0; i < vertexCount; ++i, vertexPtr += vertexSizeInBytes) {
            vertexHashes[i] = hash_mem32(vertexPtr, vertexSizeInBytes);
            vertexSorted[i] = i;
        }
    }
    ...
    STACKLOCAL_POD_ARRAY(u8, vertexOldData, vertexCount * vertexSizeInBytes);
...
void OptimizeVerticesOrder(GenericVertex& vertices, const MemoryView<u32>& indices) {
    ...
    STACKLOCAL_POD_ARRAY(u32, old2new, vertexCount);
    ...
    STACKLOCAL_POD_ARRAY(u8, tmpVertices, vertexAllSizeInBytes);
