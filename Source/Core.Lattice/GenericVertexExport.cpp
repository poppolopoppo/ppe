...
void ComputeTangentSpace(GenericVertex& vertices, const MemoryView<const u32>& indices) {
    ...
    STACKLOCAL_POD_ARRAY(float3, temporary, vertexCount*2);
    const MemoryView<float3> tangents = temporary.SubRange(0, vertexCount);
    const MemoryView<float3> binormals = temporary.SubRange(vertexCount, vertexCount);
