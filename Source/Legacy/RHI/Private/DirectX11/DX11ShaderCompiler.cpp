...
static void DX11CompileShaderBlob_(
    ...
    if ( FAILED(::D3DCompile(
        ...
        PPE_THROW_IT(FShaderCompilerException(msg, compiler, source));
    }
    Assert(!errors);
}
//----------------------------------------------------------------------------
static void DX11ReflectShaderBlob_(
    ...
    if (FAILED(::D3DReflect(
        ...
        PPE_THROW_IT(FShaderCompilerException("failed to reflect shader program", compiler, source));
    }
...
static void DX11StripShaderBlobIFN_(
    ...
    if ( FAILED(::D3DStripShader(
        ...
        PPE_THROW_IT(FShaderCompilerException("failed to strip shader program", compiler, source));
    }
...
void FDX11ShaderCompiler::PreprocessShaderSource(
    ...
    if ( FAILED(::D3DPreprocess(
        ...
        const char *msg = reinterpret_cast<const char *>(errors->GetBufferPointer());
        PPE_THROW_IT(FShaderCompilerException(msg, this, source));
    }
    Assert(!errors);
    Assert(preprocessed);
