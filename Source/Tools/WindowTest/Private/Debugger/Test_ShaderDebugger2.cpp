...
//////////////////////////////////////////////////////////////////////////////
//----------------------------------------------------------------------------
bool Test_ShaderDebugger2_(FWindowTestApp& app) {
    using namespace PPE::RHI;

    if (not EnableShaderDebugging) {
        Unused(app);
        LOG(WindowTest, Warning, L"Test_ShaderDebugger2_: skipped due to lack of debugger support (USE_PPE_RHIDEBUG={0})", USE_PPE_RHIDEBUG);
        return true;
    }
...
ARGS_IF_RHIDEBUG("Test_ShaderDebugger2_VS"));
...
ARGS_IF_RHIDEBUG("Test_ShaderDebugger2_PS"));
    ...
    TScopedResource<FGPipelineID> ppln{ fg, fg.CreatePipeline(desc ARGS_IF_RHIDEBUG("Test_ShaderDebugger2")) };
    ...
    const auto onShaderTraceReady = [&shaderOutputIsCorrect](FStringView taskName, FStringView shaderName, EShaderStages stages, TMemoryView<const FString> output) {
        ...
        if (shaderName == "Test_ShaderDebugger2_VS") {
...
27. v_Color		= g_Colors[gl_VertexIndex];
        ...
        else if (shaderName == "Test_ShaderDebugger2_PS") {
