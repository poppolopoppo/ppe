...
//----------------------------------------------------------------------------
bool FTimeline::Tick_Every(const Timespan& target, Timespan& elapsed) {
    return Tick_Every(FTimepoint::Now(), target, elapsed);
}
//----------------------------------------------------------------------------
bool FTimeline::Tick_Every(const FTimeline& other, const Timespan& target, Timespan& elapsed) {
    return Tick_Every(other._now, target, elapsed);
}
//----------------------------------------------------------------------------
bool FTimeline::Tick_Every(const FTimepoint& now, const Timespan& target, Timespan& elapsed) {
    _now = now;

    const Timespan d = FTimepoint::Duration(_last, _now);
    if (d < target)
        return false;
