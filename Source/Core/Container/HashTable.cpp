...
NO_INLINE static Pair<size_t, bool> InsertBucketUsingProbe_(
    ...
    _HashWIndex idx(hashValue, *pDataIndex);
    ...
    for (;;) {
        Assert(distance <= probe.Capacity);
        ...
        distance++;
        bucket = (bucket+1) & probe.CapacityMask;
...
static void SwapDataIndexUsingProbe_(
    ...
    Assert(bucket0 < probe.Capacity);
    Assert(bucket1 < probe.Capacity);
    Assert(bucket0 != bucket1);
    Assert(hashWIndices[bucket0]._data.data_index < probe.Size);
    Assert(hashWIndices[bucket1]._data.data_index < probe.Size);

    std::swap(  hashWIndices[bucket0]._data.data_index,
                hashWIndices[bucket1]._data.data_index );
}
//----------------------------------------------------------------------------
template <typename _HashWIndex>
...
static HashTableStats ProbingStatsUsingProbe_(
    ...
    forrange(i, 0, probe.Capacity) {
        ...
        if (not (idx.empty() || idx.deleted())) {
            const size_t d = probe.ProbeDistance(idx.hash_value(), i);
            const float f = (d - stats.MeanProbe);
            stats.DevProbe = f*f;
        }
    }
