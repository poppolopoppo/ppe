...
bool TBIHTree<T, _Traits>::Intersects(const FRay& ray) const {
    return _bih.Intersects(ray, _items.size(), Meta::MakeFunction(this, &TBIHTree::IntersectsFirstHit_));
}
//----------------------------------------------------------------------------
template <typename T, typename _Traits>
bool TBIHTree<T, _Traits>::Intersects(const FRay& ray, FHitResult* firstHit) const {
    return _bih.Intersects(ray, firstHit, _items.size(), Meta::MakeFunction(this, &TBIHTree::IntersectsFirstHit_));
}
//----------------------------------------------------------------------------
template <typename T, typename _Traits>
bool TBIHTree<T, _Traits>::Intersects(const FRay& ray, const onhit_delegate& onHit) const {
    return _bih.Intersects(ray, onHit, _items.size(), Meta::MakeFunction(this, &TBIHTree::IntersectsOnEachHit_));
}
//----------------------------------------------------------------------------
template <typename T, typename _Traits>
size_t TBIHTree<T, _Traits>::BatchIntersects(const TMemoryView<const FRay>& rays, const TMemoryView<FHitResult>& firstHits) const {
    return _bih.BatchIntersects(rays, firstHits, _items.size(), Meta::MakeFunction(this, &TBIHTree::IntersectsFirstHit_));
}
//----------------------------------------------------------------------------
template <typename T, typename _Traits>
