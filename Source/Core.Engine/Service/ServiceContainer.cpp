...
namespace {
//----------------------------------------------------------------------------
typedef Pair<Guid, IService *> ServiceEntry;
struct ServiceLessByPriority_ : public std::binary_function<const ServiceEntry&, const ServiceEntry&, bool> {
...
//----------------------------------------------------------------------------
void ServiceContainer::RegisterService(const Guid& guid, IService *service) {
    THIS_THREADRESOURCE_CHECKACCESS();
    Assert(!guid.empty());
    Assert(service);
...
//----------------------------------------------------------------------------
void ServiceContainer::UnregisterService(const Guid& guid, IService *service) {
    THIS_THREADRESOURCE_CHECKACCESS();
    Assert(!guid.empty());
    Assert(service);
...
//----------------------------------------------------------------------------
IService *ServiceContainer::RequestService(const Guid& guid) {
    THIS_THREADRESOURCE_CHECKACCESS();
    Assert(!guid.empty());
...
void ServiceContainer::Start() {
    ...
    std::stable_sort(_services.begin(), _services.end(), ServiceLessByPriority_());

    // straight order when shutting down
    for (const Pair<Guid, PService>& service : _services)
