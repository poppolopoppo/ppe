...
namespace {
//----------------------------------------------------------------------------
typedef RAWSTORAGE_THREAD_LOCAL(Mesh, char) MeshStream;
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
static bool ReadLine_Obj_ReturnEOF_(LineTokens_Obj_ *pTokens, const MeshStream& meshStream, size_t& offset) {
    Assert(pTokens);
    ...
    while (meshStream.SizeInBytes() > offset && '\n' != meshStream[offset])
        if (std::isspace(meshStream[offset])) {
            if (!comment && tokenStart != offset) {
                Assert(offset - tokenStart > 0);
                Assert(pTokens->Count < LineTokens_Obj_::Capacity);
                pTokens->Slices[pTokens->Count++] = MemoryView<const char>(meshStream.Pointer() + tokenStart, offset - tokenStart);
            }
            tokenStart = ++offset;
        }
        else if ('#' == meshStream[offset]) {
            comment = true;
            ++offset;
        }
        else {
    ...
    return (meshStream.SizeInBytes() == offset);
}
//----------------------------------------------------------------------------
...
static bool LoadMaterial_Mtl_(ModelBuilder& builder, const Filename& filename) {
    ...
    RAWSTORAGE_THREAD_LOCAL(Mesh, char) materialStream;
...
//----------------------------------------------------------------------------
static bool LoadModel_Obj_(PModel& pModel, const Filename& filename, const MeshStream& meshStream) {
    ...
    size_t offset = 0;
    do {
        eof = ReadLine_Obj_ReturnEOF_(&tokens, meshStream, offset);
        if (0 == tokens.Count)
            continue;
...
bool LoadModel(PModel& pModel, const Filename& filename) {
    ...
    if (FileSystemConstNames::ObjExt() == extname) {
        return LoadModel_Obj_(pModel, filename, meshStream);
    }
    else {
