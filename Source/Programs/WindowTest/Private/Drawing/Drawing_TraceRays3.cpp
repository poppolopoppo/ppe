...
//////////////////////////////////////////////////////////////////////////////
//----------------------------------------------------------------------------
bool Drawing_TraceRays3_(FWindowTestApp& app) {
    using namespace PPE::RHI;

    if (not (app.RHI().Features() & ERHIFeature::RayTracing)) {
        PPE_LOG(WindowTest, Warning, "Drawing_TraceRays3_: skipped due to lack of ray tracing support");
        return true;
    }
...
ARGS_IF_RHIDEBUG("Drawing_TraceRays3_RayGen"));
...
ARGS_IF_RHIDEBUG("Drawing_TraceRays3_RayMiss"));
...
ARGS_IF_RHIDEBUG("Drawing_TraceRays3_RayClosestHit"));

    const TAutoResource<FRTPipelineID> ppln{ fg.ScopedResource(fg.CreatePipeline(desc ARGS_IF_RHIDEBUG("Drawing_TraceRays3"))) };
    ...
    PPE_LOG_CHECK(WindowTest, !!cmd1);
    {
        ...
        PFrameTask tBuildGeom = cmd1->Task(FBuildRayTracingGeometry{}.SetTarget(*rtGeometry).Add(trianglesData));
        PFrameTask tBuildScene = cmd1->Task(FBuildRayTracingScene{}.SetTarget(rtScene).Add(rtInstance).DependsOn(tBuildGeom));

        fg.ReleaseResources(rtGeometry);

        PFrameTask tUpdateTable = cmd1->Task(FUpdateRayTracingShaderTable{}
            .SetTarget(rtShaders)
            .SetPipeline(ppln)
            .SetScene(rtScene)
    ...
    PPE_LOG_CHECK(WindowTest, !!cmd2);
    {
        ...
        PFrameTask tTrace = cmd2->Task(FTraceRays{}
