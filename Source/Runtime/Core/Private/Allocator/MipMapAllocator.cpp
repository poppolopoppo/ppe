...
struct FMipmapGlobalCache {
    ...
    ~FMipmapGlobalCache() {
        ...
        while (FMipmapPool* pool = Pools.PopHead()) {
            ...
            FVirtualMemory::InternalFree(pool, sizeof(FMipmapPool)
                ...
                , MEMORYDOMAIN_TRACKING_DATA(MipmapCache)
            #endif
                );
        }
    }
    ...
    FMipmapPage* GrabFreePage() NOEXCEPT {
        ...
        ON_SCOPE_EXIT([]() { // after AllocateSystem() call
            MEMORYDOMAIN_TRACKING_DATA(MipmapCache).AllocateUser(sizeof(FMipmapPage));
        });
#endif
...
#if USE_PPE_MEMORYDOMAINS
        MEMORYDOMAIN_TRACKING_DATA(MipmapCache).DeallocateUser(sizeof(FMipmapPage));
#endif
