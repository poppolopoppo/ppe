...
class TCachedMemoryPool3 : Meta::FNonCopyableNorMovable {
...
public:
    ...
    TCachedMemoryPool3() { InitializeInternalCache_(); }

    explicit TCachedMemoryPool3(_Allocator&& ralloc) : _pool(_Allocator(ralloc)) { InitializeInternalCache_(); }
    explicit TCachedMemoryPool3(const _Allocator& alloc) : _pool(alloc) { InitializeInternalCache_(); }
...
//----------------------------------------------------------------------------
template <typename _Key, typename _Value, size_t _ChunkSize, size_t _MaxChunks, typename _Allocator>
void TCachedMemoryPool3<_Key, _Value, _ChunkSize, _MaxChunks, _Allocator>::InitializeInternalCache_() {
...
//----------------------------------------------------------------------------
template <typename _Key, typename _Value, size_t _ChunkSize, size_t _MaxChunks, typename _Allocator>
TCachedMemoryPool3<_Key, _Value, _ChunkSize, _MaxChunks, _Allocator>::~TCachedMemoryPool3() {
    Assert_NoAssume(_cache.Count.load(std::memory_order_relaxed) == 0);
...
template <typename _Key, typename _Value, size_t _ChunkSize, size_t _MaxChunks, typename _Allocator>
template <typename _Ctor>
auto TCachedMemoryPool3<_Key, _Value, _ChunkSize, _MaxChunks, _Allocator>::FindOrAdd(key_type&& rkey, _Ctor&& ctor) -> TPair<index_type, bool> {
...
//----------------------------------------------------------------------------
template <typename _Key, typename _Value, size_t _ChunkSize, size_t _MaxChunks, typename _Allocator>
bool TCachedMemoryPool3<_Key, _Value, _ChunkSize, _MaxChunks, _Allocator>::Remove(index_type id) {
    return RemoveIf(id, []() constexpr -> bool { return true; });
}
template <typename _Key, typename _Value, size_t _ChunkSize, size_t _MaxChunks, typename _Allocator>
template <typename _Dtor>
bool TCachedMemoryPool3<_Key, _Value, _ChunkSize, _MaxChunks, _Allocator>::RemoveIf(index_type id, _Dtor&& dtor) {
    FCacheItem_* const pBlock = _pool.At(id);
    Assert_NoAssume(pBlock->PoolIndex() == id);
...
//----------------------------------------------------------------------------
template <typename _Key, typename _Value, size_t _ChunkSize, size_t _MaxChunks, typename _Allocator>
void TCachedMemoryPool3<_Key, _Value, _ChunkSize, _MaxChunks, _Allocator>::Clear_AssertCompletelyEmpty() {
    Assert_NoAssume(_cache.Count.load(std::memory_order_relaxed) == 0);
...
//----------------------------------------------------------------------------
template <typename _Key, typename _Value, size_t _ChunkSize, size_t _MaxChunks, typename _Allocator>
void TCachedMemoryPool3<_Key, _Value, _ChunkSize, _MaxChunks, _Allocator>::Clear_IgnoreLeaks() {
    Clear_IgnoreLeaks([]() constexpr -> bool { return true; });
}
//----------------------------------------------------------------------------
template <typename _Key, typename _Value, size_t _ChunkSize, size_t _MaxChunks, typename _Allocator>
template <typename _Dtor>
void TCachedMemoryPool3<_Key, _Value, _ChunkSize, _MaxChunks, _Allocator>::Clear_IgnoreLeaks(_Dtor&& dtor) {
    u32 segment = UINT32_MAX;
    forrange(b, 0, FHashTable_::BucketMask + 1) {
        const u32 s = (b & FHashTable_::SegmentMask);
        if (segment != s) {
...
template <typename _Key, typename _Value, size_t _ChunkSize, size_t _MaxChunks, typename _Allocator>
template <typename _Dtor>
size_t TCachedMemoryPool3<_Key, _Value, _ChunkSize, _MaxChunks, _Allocator>::GarbageCollect(FGCHandle& gc, size_t maxIterations, _Dtor&& dtor) {
...
//----------------------------------------------------------------------------
template <typename _Key, typename _Value, size_t _ChunkSize, size_t _MaxChunks, typename _Allocator>
bool TCachedMemoryPool3<_Key, _Value, _ChunkSize, _MaxChunks, _Allocator>::CheckInvariants() const {
#if USE_PPE_DEBUG
