...
//----------------------------------------------------------------------------
template <typename T, u32 _Dim>
auto TScalarBoundingBox<T, _Dim>::ClipAbove(u32 axis, T value) const -> TScalarBoundingBox {
...
//----------------------------------------------------------------------------
template <typename T, u32 _Dim>
auto TScalarBoundingBox<T, _Dim>::ClipBelow(u32 axis, T value) const -> TScalarBoundingBox {
    Assert(_min[axis] <= value && _max[axis] >= value);
    TScalarBoundingBox result = *this;
