...
struct FAllocatorBinning {
    ...
    static CONSTEXPR const u16 BinSizes[NumBins + 1] = {
        ...
        12736,  15008,  16960,  19104,  21824,  24560,  27632,  UINT16_MAX/* padding/dummy */
    };
    ...
    static u32 IndexFromSize(u16 blockSize) NOEXCEPT {
#ifdef __clang__ // codegen out-performs the explicit SSE version
        ...
        m128i_t sz0 = ::_mm_cmplt_epi16(::_mm_load_si128((m128i_t*)&BinSizes[ 0]), key);
        cnt = ::_mm_sub_epi16(cnt, sz0);

        m128i_t sz1 = ::_mm_cmplt_epi16(::_mm_load_si128((m128i_t*)&BinSizes[ 8]), key);
        cnt = ::_mm_sub_epi16(cnt, sz1);

        m128i_t sz2 = ::_mm_cmplt_epi16(::_mm_load_si128((m128i_t*)&BinSizes[16]), key);
        cnt = ::_mm_sub_epi16(cnt, sz2);

        m128i_t sz3 = ::_mm_cmplt_epi16(::_mm_load_si128((m128i_t*)&BinSizes[24]), key);
        cnt = ::_mm_sub_epi16(cnt, sz3);

        m128i_t sz4 = ::_mm_cmplt_epi16(::_mm_load_si128((m128i_t*)&BinSizes[32]), key);
        cnt = ::_mm_sub_epi16(cnt, sz4);

        m128i_t sz5 = ::_mm_cmplt_epi16(::_mm_load_si128((m128i_t*)&BinSizes[40]), key);
        cnt = ::_mm_sub_epi16(cnt, sz5);

        STATIC_ASSERT(lengthof(BinSizes) == 48);

        cnt = ::_mm_sad_epu8(cnt, ::_mm_setzero_si128());
        cnt = ::_mm_add_epi16(cnt, _mm_shuffle_epi32(cnt, _MM_SHUFFLE(1, 0, 3, 2)));

        return (_mm_cvtsi128_si32(cnt) & 0xFFFFul);
