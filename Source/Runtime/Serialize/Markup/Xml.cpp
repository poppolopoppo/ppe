    ...
    if (lexer.ReadIFN("<?xml")) {
        ...
        while (lexer.Peek(Lexer::FSymbols::Identifier)) {
            ...
            if (not lexer.Expect(Lexer::FSymbols::Assignment))
                CORE_THROW_IT(FMarkupException("expected assignment", id.Site()));

            if (not lexer.Expect(value, Lexer::FSymbols::String))
                CORE_THROW_IT(FMarkupException("expected string", id.Site()));
        ...
        if (not lexer.ReadIFN("?>"))
            CORE_THROW_IT(FMarkupException("expected closing xml header", id.Site()));
    }
...
void FXml::WriteHeaders(FTextWriter& oss, bool minify) const {
    if (not minify)
        oss << "<!-- GENERATED BY CORE.SERIALIZE/MARKUP/XML -->" << Eol;
