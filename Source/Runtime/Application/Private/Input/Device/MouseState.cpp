...
float2 FMouseState::Relative() const {
    return (_useFilteredInputs ? RelativeFiltered() : RelativeRaw());
}
//----------------------------------------------------------------------------
...
float2 FMouseState::DeltaRelative() const {
    return (_useFilteredInputs ? DeltaRelativeFiltered() : DeltaRelativeRaw());
}
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
float2 FMouseState::Wheel() const {
    return (_useFilteredInputs ? WheelFiltered() : WheelRaw());
}
//----------------------------------------------------------------------------
float2 FMouseState::WheelRaw() const {
    return float2(_wheelX.Raw(), _wheelY.Raw());
}
//----------------------------------------------------------------------------
float2 FMouseState::WheelFiltered() const {
    return float2(_wheelX.Filtered(), _wheelY.Filtered());
}
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
void FMouseState::Update(FTimespan dt) {
    _onEnter = (not _wasInside && _inside);
    _onLeave = (_wasInside && not _inside);
    _wasInside = _inside;

    _wheelX.SetRaw(_wheelX.Raw() + _wheelDeltaAccumX);
    _wheelX.Update(dt);
    _wheelDeltaAccumX = 0;

    _wheelY.SetRaw(_wheelY.Raw() + _wheelDeltaAccumY);
    _wheelY.Update(dt);
    _wheelDeltaAccumY = 0;
