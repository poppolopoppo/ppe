...
namespace {
//----------------------------------------------------------------------------
static void LinuxDisplayAdapter_(const ::DISPLAY_DEVICEW& dev, FLinuxPlatformSurvey::FDisplayAdapter* adapter) {
    Assert(adapter);
...
//----------------------------------------------------------------------------
static bool LinuxMonitorInfo_(::HMONITOR hMonitor, FLinuxPlatformSurvey::FMonitorInfo* monitor) {
    Assert(monitor);
...
//----------------------------------------------------------------------------
static bool LinuxStorageInfo_(const wchar_t* path, FLinuxPlatformSurvey::FStorageInfo* storage) {
    Assert(path);
    Assert(storage);
...
bool FLinuxPlatformSurvey::PrimaryDisplayAdapter(FDisplayAdapter* displayAdapter) {
    ...
    for (::DWORD iDevNum = 0; ; ++iDevNum) {
        ...
        if (dev.StateFlags & DISPLAY_DEVICE_PRIMARY_DEVICE) {
            LinuxDisplayAdapter_(dev, displayAdapter);
...
bool FLinuxPlatformSurvey::DisplayAdapters(FDisplayAdapters* displayAdapters) {
    ...
    for (::DWORD iDevNum = 0; ; ++iDevNum) {
        ...
        LinuxDisplayAdapter_(dev, &displayAdapters->push_back_Default());
    }
...
bool FLinuxPlatformSurvey::PrimaryMonitor(FMonitorInfo* monitor) {
    ...
    if (LinuxMonitorInfo_(NULL, monitor)) {
...
bool FLinuxPlatformSurvey::MonitorFromPoint(int x, int y, FMonitorInfo* monitor) {
    ...
    return LinuxMonitorInfo_(hMonitor, monitor);
}
//----------------------------------------------------------------------------
...
bool FLinuxPlatformSurvey::MonitorInfos(FMonitorInfos* monitors) {
    ...
    auto const addMonitor = [](::HMONITOR hMonitor, ::HDC , ::LPRECT , ::LPARAM lParam) -> ::BOOL {
        ...
        if (LinuxMonitorInfo_(hMonitor, &monitor)) {
...
bool FLinuxPlatformSurvey::ApplicationStorage(FStorageInfo* storage) {
    ...
    return LinuxStorageInfo_(*FPlatformFile::WorkingDirectory(), storage);
}
//----------------------------------------------------------------------------
bool FLinuxPlatformSurvey::SystemStorage(FStorageInfo* storage) {
    Assert(storage);

    return LinuxStorageInfo_(*FPlatformFile::SystemDirectory(), storage);
}
//----------------------------------------------------------------------------
bool FLinuxPlatformSurvey::StorageFromPath(const FWStringView& path, FStorageInfo* storage) {
    ...
    return LinuxStorageInfo_(fullname, storage);
}
//----------------------------------------------------------------------------
bool FLinuxPlatformSurvey::StorageInfos(FStorageInfos* storages) {
    ...
    for (const wchar_t* it = driveBegin; it != driveEnd; ) {
        ...
        if (LinuxStorageInfo_(drivePath.data(), &storage))
