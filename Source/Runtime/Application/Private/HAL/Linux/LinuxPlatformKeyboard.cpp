...
//----------------------------------------------------------------------------
static bool KeyboardMessageHandler_(FKeyboardState* keyboard, const FLinuxPlatformMessageHandler::FMessage& msg) {
    Assert(keyboard);

    switch (msg.Type) {
    case ELinuxMessageType::KeyDown:
    case ELinuxMessageType::SysKeyDown:
        return KeyboarSetKeyDown_(keyboard, msg.WParam);

    case ELinuxMessageType::KeyUp:
    case ELinuxMessageType::SysKeyUp:
...
//////////////////////////////////////////////////////////////////////////////
//----------------------------------------------------------------------------
FEventHandle FLinuxPlatformKeyboard::SetupMessageHandler(FLinuxWindow& window, FKeyboardState* keyboard) {
    Assert(keyboard);
    Assert(window.Handle());

    return window.OnMessageWin32().Add([keyboard](const FLinuxWindow&, FLinuxMessage* msg) {
        msg->Handled |= KeyboardMessageHandler_(keyboard, *msg);
    });
}
//----------------------------------------------------------------------------
void FLinuxPlatformKeyboard::RemoveMessageHandler(FLinuxWindow& window, FEventHandle& handle) {
    Assert(window.Handle());
    Assert(handle);
