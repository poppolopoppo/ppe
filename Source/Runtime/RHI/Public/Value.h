...
#include "Value.Definitions-inl.h"
...
FOREACH_PPE_GRAPHIC_VALUETYPE(VALUETYPE_ENUM_DEF)
...
FOREACH_PPE_GRAPHIC_VALUETYPE(VALUETYPE_UNION_DEF)
...
FOREACH_PPE_GRAPHIC_VALUETYPE(VALUETYPE_TRAITS_DEF)
...
void FValue::Set(const T& value) {
    ...
    INPLACE_NEW(std::addressof(TValueTraits<T>::Get(_data)), T)(value);
}
//----------------------------------------------------------------------------
template <typename T>
